...
class AbsSignalGen(SocIp):
    ...
    def configure_connections(self, soc):
        ...
        block, port, _ = soc.metadata.trace_forward(
            self["fullpath"], "m_axis", ["usp_rf_data_converter"]
        )
...
class AbsArbSignalGen(AbsSignalGen):
    ...
    def load(self, xin, addr=0):
        ...
        if length + addr > self.MAX_LENGTH:
            raise RuntimeError(
                "%s: buffer length must be %d samples or less."
        ...
        np.copyto(self.buff[:length], np.frombuffer(xin, dtype=np.int32))
    ...
    * 1 : enable writes.
    ...
    def __init__(self, description):
        ...
        for i in range(self.N_TONES):
            self.REGISTERS["pinc%d_reg" % (i)] = i + iReg
        iReg += self.N_TONES
        if self.HAS_PHASE:
            for i in range(self.N_TONES):
                self.REGISTERS["poff%d_reg" % (i)] = i + iReg
            iReg += self.N_TONES
        if self.HAS_GAIN:
            for i in range(self.N_TONES):
                self.REGISTERS["gain%d_reg" % (i)] = i + iReg
            iReg += self.N_TONES
    ...
    def set_tones(self, freqs, gains=None, phases=None, ro_ch=None):
        ...
        if self.HAS_MIXER:
            ...
            mixer_freq = 0
        tones = self.soc.calc_muxgen_regs(
            self.ch, freqs, gains, phases, ro_ch, False, mixer_freq
        )
