    ...
    * 1 : Periodic.
    ...
    def configure_connections(self, soc):
        ...
        block, port, _ = soc.metadata.trace_back(
            self["fullpath"], "s_axis", ["usp_rf_data_converter"]
        )
...
class AbsPFBReadout(SocIp, AbsReadout):
    ...
    def configure_connections(self, soc):
        ...
        # what RFDC port drives this readout?
        block, port, blocktype = soc.metadata.trace_back(
            self["fullpath"], "s_axis", ["usp_rf_data_converter", "axis_combiner"]
        )
    ...
    * IDx_REG   : lower 8 bits are the "packet" field, from 0 .. 7 (N/L).
    ...
    def __init__(self, description):
        ...
        for i in range(self.NOUT):
            self.REGISTERS["id%d_reg" % (i)] = i + iReg
        iReg += self.NOUT
        for i in range(self.NOUT):
    ...
    def set_freq_int(self, cfg):
        ...
        if pfb_ch not in range(self.NCH):
            raise RuntimeError(
                "Invalid PFB channel: %d. It must be within [0, %d]"
                % (pfb_ch, self.NCH - 1)
            )
        # Check output channel is within allowed range.
        if out_ch not in range(self.NOUT):
            raise RuntimeError(
                "Invalid %d output channel. It must be within [0, %d]"
                % (out_ch, self.NOUT - 1)
            )
...
class AbsDynReadout(AbsReadout):
    ...
    def configure_connections(self, soc):
        ...
        # what RFDC port drives this readout?
        block, port, _ = soc.metadata.trace_back(
            self["fullpath"], "s1_axis", ["usp_rf_data_converter"]
        )
...
class AxisAvgBuffer(SocIp):
    ...
    * 1 : send data using m1_axis.
    ...
    def configure_connections(self, soc):
        ...
        self.cfg["trigger_type"], self.cfg["trigger_port"], self.cfg["trigger_bit"] = (
            _trace_trigger(soc, self.fullpath)
        )

        # which tProc input port does this buffer drive?
        try:
            block, port, _ = soc.metadata.trace_forward(
                self["fullpath"], "m2_axis", ["axis_tproc64x32_x8", "qick_processor"]
            )
            # ask the tproc to translate this port name to a channel number
    ...
    def set_freq(self, f, gen_ch=0):
        ...
        """
        if isinstance(
            self.readout, (AxisPFBReadoutV2, AxisPFBReadoutV3, AxisPFBReadoutV4)
        ):
    ...
    def transfer_avg(self, address=0, length=100):
        ...
        if length >= self["avg_maxlen"]:
            raise RuntimeError(
                "length=%d longer than %d" % (length, self["avg_maxlen"])
            )
        ...
        if self.dma_avg.recvchannel.transferred != transferlen * 8:
            raise RuntimeError(
                "Requested %d samples but only got %d from DMA"
    ...
    def config_buf(self, address=0, length=100):
        ...
        if length >= self["buf_maxlen"]:
            raise RuntimeError(
                "requested length=%d longer or equal to decimated buffer size=%d"
...
class MrBufferEt(SocIp):
    ...
    def configure_connections(self, soc):
        ...
        block, port, blocktype = soc.metadata.trace_back(
            self["fullpath"], "s00_axis", ro_types + ["axis_switch"]
        )
        ...
        if blocktype == "axis_switch":
            ...
            for iIn in range(NUM_SI_param):
                ...
                block, port, blocktype = soc.metadata.trace_forward(
                    ro_block, "m1_axis", ["axis_avg_buffer"]
                )
...
class AxisBufferDdrV1(SocIp):
    ...
    def __init__(self, description):
        # Generics.
        self.TARGET_SLAVE_BASE_ADDR = int(
            description["parameters"]["TARGET_SLAVE_BASE_ADDR"], 0
        )
        self.ID_WIDTH = int(description["parameters"]["ID_WIDTH"])
        self.DATA_WIDTH = int(
            description["parameters"]["DATA_WIDTH"]
        )  # width of the AXI bus, in bits
        self.BURST_SIZE = (
            int(description["parameters"]["BURST_SIZE"]) + 1
        )  # words per AXI burst
        ...
        self.cfg["junk_len"] = (
            50 * self.DATA_WIDTH // 32 + 1
        )  # not clear where this 50 comes from, presumably some FIFO somewhere
    ...
    def configure_connections(self, soc):
        ...
        block, port, blocktype = soc.metadata.trace_back(
            self["fullpath"], self.STREAM_IN_PORT, RO_TYPES + ["axis_switch"]
        )
    ...
    def get_mem(self, nt, start=None):
        ...
        return (
            buf_copy[start % 2 : length + start % 2]
            .view(dtype=np.int16)
            .reshape((-1, 2))
        )
    ...
    def arm(self, nt, force_overwrite=False):
        if nt > self["maxlen"] // self["burst_len"] and not force_overwrite:
            raise RuntimeError(
                "the requested number of DDR4 transfers (nt) exceeds the memory size; the buffer will overwrite itself. You can disable this error message with force_overwrite=True."
            )
        self.wlen(nt)
        self.wstop()
        self.wstart()
