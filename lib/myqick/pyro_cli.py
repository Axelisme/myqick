...
for opt, arg in options:
    if opt == "-n":
        ...
        print("\n")
        print(
            "On pynq 2.7 you may get an error relating to pynq not loading, or no XRT devices being found."
        )
        print(
            "If this happens, try the following (taken from /usr/local/bin/start_jupyter.sh):"
        )
        print("\tsudo -s")
