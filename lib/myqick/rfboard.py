...
class BiasAD5781:
    ...
    def __init__(self, spi_ip, ch_en, cs_t=""):
        ...
        logger.debug(
            "{}: DAC Channel = {}.".format(self.__class__.__name__, self.ch_en)
        )
    ...
    def _volt2reg(self, volt):
        if volt < self.VREFN or volt > self.VREFP:
            raise RuntimeError(
                "%s: %d V out of range [%f, %f]"
                % (self.__class__.__name__, volt, self.VREFN, self.VREFP)
            )
    ...
    def read_reg(self, reg):
        ...
        if (res >> 20) != addr:
            logger.error(
                "AD5781 readback failed: tried to read addr %d, got back 0x%x"
                % (addr, res >> 20)
            )
        return res & 0x0FFFFF
    ...
    def write_reg(self, reg, val):
        ...
        logger.debug(
            "{}: writing register {} with values {}.".format(
                self.__class__.__name__, reg, msg
            )
        )
...
class AttenuatorPE43705:
    ...
    def _db2step(self, db):
        ...
        if db < self.dbMinAtt or db > self.dbMaxAtt:
            raise RuntimeError(
                "attenuation value %f out of range [%f, %f]"
                % (db, self.dbMinAtt, self.dbMaxAtt)
            )
...
class FilterADMV8818:
    ...
    def write_reg(self, reg, value):
        ...
        # Data.
        msg = (addr & 0x7FFF).to_bytes(length=2, byteorder="big") + value.to_bytes(
    ...
    def set_filter(self, fc=0, bw=2.0, ftype="lowpass"):
        ...
        value = (
            0xC0
            + (self._band2switch(section="HPF", band=band_hpf) << 3)
            + self._band2switch(section="LPF", band=band_lpf)
        )
...
class SwitchControl:
    ...
    def add_MCP(self, gpio, outputs):
        if len(outputs) != len(gpio.outputs):
            raise RuntimeError(
                "must define all %d outputs from the MCP23S08 (use None for NC pins)"
                % (len(gpio.outputs))
            )
        defaults = 0
    ...
    def set_bits(self, bits, val):
        ...
        for bit in bits:
            if bit not in self.outputs:
                raise RuntimeError(
                    "tried to set output %d, but only pins %s are configured as outputs"
                    % (bit, self.outputs)
                )
            if val == 1:
                reg |= 1 << bit
...
class LoSynthADF4372:
    ...
    def set_freq(self, fin=6000):
        ...
        if fin < 4000 or fin > 8000:
            raise RuntimeError(
                "%s: input frequency %d out of range" % (self.__class__.__name__, fin)
            )
...
class GainLMH6401:
    ...
    def set_gain(self, db):
        ...
        if db < self.Gmin or db > self.Gmax:
            raise RuntimeError(
                "%s: gain %f out of limits [%f, %f]"
                % (self.__class__.__name__, db, self.Gmin, self.Gmax)
            )
...
class AdcDcChain111(AbsAdcDcChain):
    ...
    def disable(self):
        if self.powerdown is not None:
            ...
            raise RuntimeError(
                "enable/disable only supported on ZCU111 V2, is this V1?"
            )
...
class DacChain111(AbsDacRfChain, AbsDacDcChain):
    ...
    def rfsw_sel(self, sel="RF"):
        if sel == "RF":
            ...
            raise RuntimeError(
                "%s: selection %s not recoginzed." % (self.__class__.__name__, sel)
            )
    ...
    def __init__(self, soc, card, global_ch, card_num, card_ch):
        ...
        logger.debug(
            "{}: adding filter with address {}.".format(
                self.__class__.__name__, card_ch
            )
        )
            ...
            if self.CHAIN_CLASS is not None:
                self.chains.append(
                    self.CHAIN_CLASS(
                        ...
                        card=self,
                        global_ch=global_ch,
                        card_num=self.CARDNUM_OFFSET + card_num,
                        card_ch=card_ch,
                    )
                )
            else:
...
class BoardSelection:
    ...
    def enable(self, board_id=0):
        ...
        logger.debug(
            "{}: setting vaue = 0x{:01X}".format(self.__class__.__name__, val_)
        )
...
class RFQickSoc111V1(RFQickSoc):
    def _init_switches(self, spi):
        ...
        self.switches.add_MCP(
            GpioMCP23S08(spi, ch_en=0, dev_addr=0),
            outputs=[("RF2IF5V_EN" + str(i), 0) for i in range(4)] + [None] * 4,
        )
        # DAC power
        self.switches.add_MCP(
            GpioMCP23S08(spi, ch_en=1, dev_addr=0),
            outputs=[("IF2RF5V_EN" + str(i), 0) for i in range(8)],
        )
        # DAC RF/DC switch
        self.switches.add_MCP(
            GpioMCP23S08(spi, ch_en=2, dev_addr=0),
    ...
    def rfb_config(self, no_tproc):
        ...
        # ADC channels.
        self.adc_chains = [
            AdcRfChain111(ii, self.switches, self.attn_spi) for ii in range(4)
        ] + [AdcDcChain111(ii, self.switches, self.psf_spi) for ii in range(4, 8)]

        # DAC channels.
        self.dac_chains = [
            DacChain111(ii, self.switches, self.attn_spi) for ii in range(8)
        ]
...
class RFQickSoc111V2(RFQickSoc111V1):
    def _init_switches(self, spi):
        ...
        # DAC power-down
        self.switches.add_MCP(
            GpioMCP23S08(spi, ch_en=1, dev_addr=1),
            ...
            for slot, card in enumerate(self.dac_cards):
                if card is None:
                    ...
                    try:
                        ...
                        channels = "[UNKNOWN]"
                    self["extra_description"].append(
                        f"\tslot {slot}: DAC card {type(card)} has channels {channels}"
                    )
            ...
            for raw_slot, card in enumerate(self.adc_cards):
                ...
                if card is None:
                    ...
                    try:
                        ...
                        channels = "[UNKNOWN]"
                    self["extra_description"].append(
                        f"\tslot {slot}: ADC card {type(card)} has channels {channels}"
                    )
