...
class Logger:
    ...
    def setLevel(level: int) -> None:
        if level > 2:
            raise ValueError(
                "Logger.setLevel: show_level must be samller than 2 (0-INFO, 1-WARNINGS, 2-ERRORS)"
            )
        Logger.level = level
        ...
        if Type == "Dest":
            if REG[0]:  ## is SREG
                if int(REG[0]) > 15:
                    error = Logger.error(
                        "get_reg_addr",
                        "Register s" + str(REG[0]) + " is not a sreg (Max 15)",
                    )
                else:
                ...
                if int(REG[1]) > 31:
                    error = Logger.error(
                        "get_reg_addr",
                        "Register d" + str(REG[1]) + " is not a dreg (Max 31)",
                    )
                else:
                ...
                if int(REG[2]) > 5:
                    error = Logger.error(
                        "get_reg_addr",
                        "Register w" + str(REG[2]) + " is not a wreg (Max 5)",
                    )
                else:
            ...
            if REG[0]:  ## is SREG
                ...
                if int(REG[2]) > 5:
                    error = Logger.error(
                        "get_reg_addr",
                        "Register w" + str(REG[1]) + " is not a wreg (Max 5)",
                    )
                else:
...
class Assembler:
                ...
                if command:
                    ...
                    if label:  # add label to label_dict if not already registered.
                        ...
                        if check_name(L_Name):
                            ...
                            error = Logger.error(
                                "LABEL_RECOGNITION",
                                "Label Name error in line  " + str(line_number),
                            )
                        ...
                        if directive == "ALIAS":
                                ...
                                if check_name(A_Name):
                                    if A_Name in Alias_List:
                                        ...
                                        if check_reg(A_Reg):
                                            ...
                                            Logger.info(
                                                "ALIAS_RECOGNITION",
                                                " > " + A_Reg + " is called " + A_Name,
                                            )
                                        else:
                                            error = Logger.error(
                                                "DIRECTIVE_RECOGNITION",
                                                "Register Name error in line "
                                                + str(line_number),
                                            )
                                else:
                                    error = Logger.error(
                                        "DIRECTIVE_RECOGNITION",
                                        "Alias Name Error in line " + str(line_number),
                                    )
                            else:
                                error = Logger.error(
                                    "DIRECTIVE_RECOGNITION",
                                    "ALIAS Parameters error in line "
                                    + str(line_number),
                                )
                            ...
                            if len(directive_params) == 3:
                                ...
                                error = Logger.error(
                                    "DIRECTIVE_RECOGNITION",
                                    "CONST Parameters error in line "
                                    + str(line_number),
                                )
                            ...
                            if len(directive_params) == 2:
                                ...
                                error = Logger.error(
                                    "DIRECTIVE_RECOGNITION",
                                    "ADDR Parameters error in line " + str(line_number),
                                )
                        elif directive == "END":
                            mem_addr += 1
                        else:
                            error = Logger.error(
                                "DIRECTIVE_RECOGNITION",
                                "Directive Not Recognized in Line " + str(line_number),
                            )
                    elif (
                        instruction
                        ...
                        if instruction in instList.keys():
                            ...
                            error = Logger.error(
                                "CMD_RECOGNITION",
                                "Command Not Recognized in Line " + str(line_number),
                            )
                    else:
                        error = Logger.error(
                            "CMD_RECOGNITION",
                            "Instruction Not Recognized in Line " + str(line_number),
                        )
                ...
                if (not command) or (line_number in label_line_idxs):
                    ...
                    if instruction in instList.keys():
                        ...
                        if LIT and len(LIT) == 2 and LIT[0] != LIT[1]:
                            error = Logger.error(
                                "COMMAND_RECOGNITION",
                                "Literals not equals in Line " + str(line_number),
                            )
                        ...
                        if error == 0:
                            ...
                            if "OP" in command_info:
                                ...
                                if param_op:
                                    try:
                                        ...
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Binary value incorrect in Line "
                                            + str(line_number),
                                        )
                            if "LIT" in command_info:
                                # Remove underscores
                                command_info["LIT"] = command_info["LIT"].replace(
                                    "_", ""
                                )
                                # Check if Binary OK
                                ...
                                if len(CMD_DEST_SOURCE) > 2:
                                    if CMD_DEST_SOURCE[1] == "r_wave":
                                        ...
                                        if "WP" in command_info:
                                            error = Logger.error(
                                                "COMMAND_RECOGNITION",
                                                "Not allowed Write Port < -wp() > in Line "
                                                + str(line_number),
                                            )
                                        if "WR" in command_info:
                                            error = Logger.error(
                                                "COMMAND_RECOGNITION",
                                                "Not allowed Write Register < -wr() > in Line "
                                                + str(line_number),
                                            )
                                        if "WW" in command_info:
                                            error = Logger.error(
                                                "COMMAND_RECOGNITION",
                                                "Not allowed Write WaveMemory < -ww() > in Line "
                                                + str(line_number),
                                            )
                                        if "TIME" in command_info:
                                            error = Logger.error(
                            ...
                            if (
                                (CMD_DEST_SOURCE[0] == "NOP")
                                or (CMD_DEST_SOURCE[0] == "TEST")
                                or (CMD_DEST_SOURCE[0] == "RET")
                                or (CMD_DEST_SOURCE[0] == "TIME")
                                or (CMD_DEST_SOURCE[0] == "FLAG")
                                or (CMD_DEST_SOURCE[0] == "ARITH")
                                or (CMD_DEST_SOURCE[0] == "DIV")
                                or (CMD_DEST_SOURCE[0] == "NET")
                                or (CMD_DEST_SOURCE[0] == "COM")
                                or (CMD_DEST_SOURCE[0] == "PA")
                                or (CMD_DEST_SOURCE[0] == "PB")
                            ):
                                if "WP" in command_info:
                                    error = Logger.error(
                                        "COMMAND_RECOGNITION",
                            ...
                            ###########################################################
                            if (CMD_DEST_SOURCE[0] == "JUMP") or (
                                CMD_DEST_SOURCE[0] == "CALL"
                            ):
                                if "WP" in command_info:
                                    error = Logger.error(
                                        "COMMAND_RECOGNITION",
                                ...
                                if "TIME" in command_info:
                                    error = Logger.error(
                                        "COMMAND_RECOGNITION",
                                        "DMEM_WR is NOT a timed intruction < -@Time > in Line "
                                        + str(line_number),
                                    )
                            ...
                            ###########################################################
                                ...
                                if "TIME" in command_info:
                                    if "WR" in command_info:
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Not allowed SDI with Literal Time in Line "
                                            + str(line_number),
                                        )
                                    if "OP" in command_info:
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Not allowed ALU Operation Operation with Literal Time in Line "
                                            + str(line_number),
                                        )
                                if ("WP" in command_info) and not (
                                    "PORT" in command_info
                                ):
                                    error = Logger.error(
                                        "COMMAND_RECOGNITION",
                        ...
                        if error == 0:
                            ...
                            if len(CMD_DEST_SOURCE) > 3:
                                if (
                                    CMD_DEST_SOURCE[0] == "ARITH"
                                    or CMD_DEST_SOURCE[0] == "NET"
                                ) or (
                                    CMD_DEST_SOURCE[0] == "PA"
                                    or CMD_DEST_SOURCE[0] == "PB"
                                ):
                                    ...
                                    if len(CMD_DEST_SOURCE) > 5:
                                        ...
                                        if CMD_DEST_SOURCE[0] == "NET":
                                            error = Logger.error(
                                                "COMMAND_RECOGNITION",
                                                "NET command max 3 Registers in line "
                                                + str(line_number),
                                            )
                            ...
                            ## ONLY ONE SOURCE / DEST
                                ...
                                if CMD_DEST_SOURCE[0] == "REG_WR":
                                    if CMD_DEST_SOURCE[2] == "label":
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Missing label in line " + str(line_number),
                                        )
                                    else:
                                        command_info["DST"] = CMD_DEST_SOURCE[1]
                                        command_info["SRC"] = CMD_DEST_SOURCE[2]
                                    ...
                                    if int(command_info["PORT"]) > 3:
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Data Port MAX port number is p3 in line "
                                            + str(line_number),
                                        )
                                    else:
                            ...
                            ## NO SOURCE OR -- SOURCE IN EXTRACTED PARAMETER
                                ...
                                if CMD_DEST_SOURCE[0] == "DMEM_WR":
                                    ...
                                    if int(command_info["PORT"]) > 31:
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Trigger Port max por number is p31 in line "
                                            + str(line_number),
                                        )
                                    else:
                                        command_info["DST"] = command_info["PORT"]
                                    ...
                                    if int(command_info["PORT"]) > 15:
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Wave Port Port max value is 15 in line "
                                            + str(line_number),
                                        )
                                    else:
                                        command_info["DST"] = command_info["PORT"]
                                    ...
                                    if "LIT" in command_info:
                                        ...
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Dividend Parameter Error in line "
                                            + str(line_number),
                                        )
                                elif (
                                    CMD_DEST_SOURCE[0] == "JUMP"
                                    or CMD_DEST_SOURCE[0] == "CALL"
                                ):
                                    ...
                                    Logger.info(
                                        "COMMAND_RECOGNITION", "WAIT adding Instruction"
                                    )
                                    command_info["C_OP"] = CMD_DEST_SOURCE[1]
                                    command_info["P_ADDR"] = mem_addr
                                    ...
                                    command_info["C_OP"] = CMD_DEST_SOURCE[1]
                                    Logger.info(
                                        "COMMAND_RECOGNITION", "CLEAR Instruction"
                                    )
                                    command_info["P_ADDR"] = mem_addr
                            ...
                            ## NO DESTINATION OR -- DESTINATION / SOURCE IN EXTRACTED PARAMETER
                                ...
                                elif (CMD_DEST_SOURCE[0] == "JUMP") or (
                                    CMD_DEST_SOURCE[0] == "CALL"
                                ):
                                    if "ADDR" in command_info:
                                        ...
                                        error = Logger.error(
                                            "COMMAND_RECOGNITION",
                                            "Address Parameter Error in line "
                                            + str(line_number),
                                        )
                                else:
                                    error = Logger.error(
                                        "COMMAND_RECOGNITION",
                                        "[1] Parameter Error in line "
                                        + str(line_number),
                                    )
                            else:
                                error = (
                                    Logger.error(
                                        "COMMAND_RECOGNITION",
                                        "Error Processing Line " + str(line_number),
                                    )
                    ...
                    error = Logger.error(
                        "COMMAND_RECOGNITION",
                        "Not a Command in Line " + str(line_number),
                    )
        ...
        for command in program_list:
            if "CMD" in command:
                ...
                ###############################################################################
                    ...
                    error = Logger.error(
                        "COMMAND_TRANSLATION",
                        "Command Listed but not programmed > " + command["CMD"],
                    )
            else:
                error = Logger.error(
            ...
            if length != 72:
                if command["CMD"] == "WAIT":
                    Logger.info(
                        "COMMAND_TRANSLATION",
                        "Command Wait add one more instruction " + str(command["LINE"]),
                    )
                else:
                    error = 72
                    Logger.error(
    ...
    def file_asm2bin(filename: str, save_unparsed_filename: str = "") -> list:
        ...
        if program_list:
            binary_program_list = Assembler.list2bin(
                program_list, save_unparsed_filename
            )
...
class Instruction:
            ...
            if len(DEST_SOURCE) == 2:
                if DEST_SOURCE[1] == "op":
                    if "OP" in command:
                        ...
                        error = Logger.error(
                            "Parameter.WR",
                            "Operation < -op() > option not found in instruction "
                            + str(command["LINE"]),
                        )
    ...
    def __PROCESS_WP(command: dict) -> tuple:
        ...
        if "WP" in command:
            ...
            if "PORT" in command:
                ...
                error = Logger.error(
                    "Parameter.WP",
                    "Port Address not recognized < pX > " + str(command["LINE"]),
                )
    ...
    def __PROCESS_SOURCE(command: dict) -> tuple:
            ...
            if len(cmd_op) == 1:  # Operation is COPY REG (Add Zero)
                ...
                if error == 0:
                    if src_type[0] == "R":
                        ...
                        error = Logger.error(
                            "Parameter.SRC", "Operand can not be a Literal."
                        )
                ...
                if FULL:
                    if operation in aluList_op:  # ALU LIST ONE PARAMETER
                        ...
                        error = Logger.error(
                            "Parameter.SRC",
                            "Operation Not Recognized > " + str(command["OP"]),
                        )
                ...
                if error == 0:
                    ...
                    if src_type[0] == "R":  ## REG OP REG
                        ...
                        if error:
                            error = Logger.error(
                                "Parameter.SRC",
                                "Literal Value error in instruction "
                                + str(command["LINE"]),
                            )
                        else:
                            if (
                                ...
                                if error == 0:
                                    if lit_val > 15:
                                        error = Logger.error(
                                            "Parameter.SRC",
                                            "Max Shift is 15 in instruction "
                                            + str(command["LINE"]),
                                        )
                                else:
                                    error = Logger.error(
        ...
        if error == 0:
            ...
            error = Logger.error(
                "Parameter.SRC", "Error in line " + str(command["LINE"])
            )
            return error, "X", "X", "X"
    ...
    def __PROCESS_MEM_ADDR(ADDR_CMD: str) -> tuple:
        ...
        if len(param_op) == 1:
            ...
            if param_op[0][0]:  ## is SREG
                ...
                error = Logger.error(
                    "Parameter.MEM_ADDR", "First Operand not recognized."
                )
        elif len(param_op) == 3:
    ...
    def REG_WR(current: dict) -> tuple:
        ...
        if error == 0:
            ...
            #### SOURCE LABEL
                ...
                if "ADDR" in current:
                    ...
                    if address[0]:  # LITERAL
                        ...
                        error = Logger.error(
                            "Instruction.REG_WR",
                            "Address error in line " + str(current["LINE"]),
                        )
                else:
                    error = Logger.error(
                        "Instruction.REG_WR",
            ...
            #### SOURCE DATA MEMORY
                ...
                if error == 0:
                    if "ADDR" in current:
                        error, rsA0, rsA1, AI = Instruction.__PROCESS_MEM_ADDR(
                            current["ADDR"]
                        )
                        ADDR = rsA0 + "__" + rsA1
                        CFG = "01__" + current["UF"] + "_00_" + alu_op
                        ...
                        error = Logger.error(
                            "Instruction.REG_WR",
                            "No Address for dmem in line " + str(current["LINE"]),
                        )
            #### SOURCE WAVE MEM
        ...
        if error == 0:
            if current["SRC"] == "op":
                ...
                if "LIT" in current:
                    ...
                    error = Logger.error(
                        "Instruction.MEM_WR",
                        "No Literal value found in instruction " + str(current["LINE"]),
                    )
            else:
                error = Logger.error(
                    "Instruction.MEM_WR",
    ...
    def WMEM_WR(current: dict) -> tuple:
        ...
        if "DST" in current:
            ...
            error = Logger.error(
                "Instruction.WMEM_WR",
                "No address specified in line " + str(current["LINE"]),
            )
        #### WRITE REGISTER
        Wr = Rdi = "0"
        if error == 0:
        ...
        if error == 0:
            if "TIME" in current:
                ...
                if error:
                    error = Logger.error(
                        "Parameter.WMEM_WR", "Time Value error in instruction "
                    )
                else:
                    DATA = "_____" + TIME
            ...
            error = Logger.error(
                "Instruction.WMEM_WR", "Error in line " + str(current["LINE"])
            )
            CODE = "X"
        return error, CODE
    ...
    def CFG(current: dict) -> tuple:
        ...
        if error == 0:
            ...
            error = Logger.error(
                "Instruction.CFG", "Error in line " + str(current["LINE"])
            )
            CODE = "X"
        return error, CODE
    ...
    def BRANCH(current: dict, cj: str) -> tuple:
        ...
        if error == 0:
            ...
            Logger.error(
                "Instruction.BRANCH",
                "Exit with Error in instruction " + str(current["LINE"]),
            )
            CODE = "X"
        return error, CODE
    ...
    def PORT_WR(current: dict) -> tuple:
        error = 0
        ##### DATA PORTS
        if (current["CMD"] == "DPORT_WR" or current["CMD"] == "DPORT_RD") or (
            current["CMD"] == "TRIG"
        ):
            SO = AI = Ww = Sp = "0"
            rsA0 = "___00000000000"
                    ...
                    if current["SRC"] == "imm":
                        if "DATA" in current:
                            if int(current["DATA"]) > 2047:
                                error = Logger.error(
                                    "Instruction.PORT_WR",
                                    "Data imm should be smaller than 2047 No Port Data value found in line "
                                    + str(current["LINE"]),
                                )
                            else:
                                AI = Sp = "1"
                        ...
                        if "DATA" in current:
                            ...
                            if param_op:
                                ...
                                error = Logger.error(
                                    "Instruction.PORT_WR",
                                    "Register Selection Error, should be dreg in line "
                                    + str(current["LINE"]),
                                )
                        else:
                            error = Logger.error(
        ...
        if error == 0:
            if "TIME" in current:
                ...
                if "WR" in current or "OP" in current:
                    error = Logger.error(
                        "Instruction.PORT_WR",
                        "If time specified, Not allowed SDI <-wr(), -op()> in line "
                        + str(current["LINE"]),
                    )
            else:
                TO = "0"
                error = Logger.info(
                    "Instruction.PORT_WR",
                    "No time specified for command will use s_time in line "
                    + str(current["LINE"]),
                )
                #### WRITE REGISTER
                Wr = Rdi = "0"
        ...
        if error == 0:
            if "DST" in current:
                ...
                error = Logger.error(
                    "Instruction.PORT_WR",
                    "No Destination Port in line " + str(current["LINE"]),
                )
        if error == 0:
            COND = Ww + Sp + Wp
            ADDR = rsA0 + "__" + rsA1
            ...
            Logger.error(
                "Instruction.PORT_WR", "Exit with Error in line " + str(current["LINE"])
            )
            CODE = "X"
        return error, CODE
    ...
    def CTRL(current: dict) -> tuple:
        ...
        if current["CMD"] == "TIME":
            ...
            if current["C_OP"] == "rst":
                ...
                error = Logger.error(
                    "Instruction.CTRL",
                    "Posible Operations for TIME command are (rst, set_ref, inc_ref)",
                )
            if "LIT" in current:
                DF = "11"
                RD0 = "_"
        ...
        ######### FLAG
            ...
            if current["C_OP"] == "set":
                ...
                error = Logger.error(
                    "Instruction.CTRL",
                    "Posible Operations for FLAG command are (set, clr, inv)",
                )
        ######### DIVISION
            ...
            if error == 0:
                if check_reg(current["DEN"]):  # Is Register
                    ...
                    error = Logger.error(
                        "Instruction.CTRL",
                        "DIV Denominator not recognized in line "
                        + str(current["LINE"]),
                    )
        ######### NET
        ...
        ######### COM
            ...
            if current["C_OP"] == "set_flag":
                if current["R1"] == "0":
                    ...
                    error = Logger.error(
                        "Instruction.CTRL", "COM flag value can be 0 or 1"
                    )
                ...
                if current["C_OP"] == "set_byte_1":
                    ...
                    error = Logger.error(
                        "Instruction.CTRL",
                        "Posible Operations for COM command are (set_flag, set_byte, set_hw, set_word)",
                    )
                if "LIT" in current:
                    DF = "11"
                    RD0 = ""
        ...
        ######### CUSTOM Peripheral
            ...
            if int(current["C_OP"]) > 31:
                error = Logger.error(
                    "COMMAND_RECOGNITION",
                    "External Peripheral Operation not in range [0:31] in line "
                    + str(current["LINE"]),
                )
            else:
                ...
                if "LIT" in current:
                    error = Logger.error(
                        "Instruction.CTRL",
                        "No Immediate value allowed in Peripheral instruction",
                    )
                else:
                    if "R1" in current:
        ...
        if error:
            Logger.error(
                "Instruction.CTRL", "Error in instruction " + str(current["LINE"])
            )
            CODE = "X"
        else:
            CODE = (
    ...
    def ARITH(current: dict) -> tuple:
        ...
        if error == 0:
            ...
            Logger.error(
                "Instruction.ARITH", "Exit with Error in line " + str(current["LINE"])
            )
            CODE = "X"
        return error, CODE
    ...
    def CLEAR(current: dict) -> tuple:
        ...
        if current["C_OP"] == "arith":
            ...
            error = Logger.error(
                "Instruction.CLEAR",
                "No Recognized Operation in line " + str(current["LINE"]),
            )
        if error == 0:
